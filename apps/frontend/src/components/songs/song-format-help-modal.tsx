...
export function SongFormatHelpModal({
  open,
  onClose,
}: SongFormatHelpModalProps) {
  React.useEffect(() => {
    if (!open) return;
    ...
    >
      ...
      >
        <div className="shrink-0 border-b border-border px-5 py-4">
          <h2
            id="song-format-help-title"
            className="text-lg font-semibold text-card-foreground"
          >
            คำแนะนำการใส่ ChordPro
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            ถ้าต้องการให้ preview แสดงคอร์ดตรงตำแหน่ง
            ให้ใส่คอร์ดในวงเล็บเหลี่ยมก่อนคำร้องที่ต้องการผูก
          </p>
        </div>
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4 text-sm">
          <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
            ...
            <li>
              หนึ่งบรรทัดสามารถมีหลายคอร์ดได้ โดยใส่คอร์ดก่อนข้อความแต่ละช่วง
            </li>
            <li>
              ถ้ามีไทยและอังกฤษ ให้แยกเป็นคนละบรรทัด (ลำดับเดียวกันต่อท่อน)
            </li>
