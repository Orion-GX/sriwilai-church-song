...
เอกสารนี้สรุป **ชุดทดสอบแบบ end-to-end** ของ API (และบางโมดูลมี WebSocket) ใน `apps/backend/test/` พร้อมวิธีเตรียมสภาพแวดล้อมและรันคำสั่ง

## สิ่งที่ต้องมีก่อนรัน

1. **PostgreSQL** ฐานข้อมูลสำหรับเทส (แยกจาก dev ถ้าเป็นไปได้)
2. **ไฟล์ `apps/backend/.env.test`** — คัดลอกจาก `.env.test.example` แล้วตั้งค่า `DB_*` ให้ตรงกับ Postgres ที่ใช้ทดสอบ
3. **รัน migration บนฐานเทส** (จากโฟลเดอร์ `apps/backend` หรือ `yarn workspace @app/backend …` จากราก):
...
- คัดลอกบล็อก env ที่สคริปต์พิมพ์ตอนจบไปที่ `apps/frontend/.env.test`
